...
const CompetencySlider: React.FC<{
    ...
    return (
        <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-md border border-slate-200 dark:border-slate-700">
            <label htmlFor={name} className="block text-lg font-semibold text-slate-800 dark:text-slate-200">{name}</label>
            <div className="flex items-center mt-3">
                ...
                />
                <span className="ml-4 w-28 text-center text-primary-600 dark:text-primary-400 font-semibold">{ratingLabels[value]}</span>
            </div>
        </div>
    );
};
        ...
        } catch (err: any)
{
    ...
    return (
        <div className="flex flex-col h-full animate-fade-in">
            <div className="mb-6">
                <h1 className="text-4xl font-bold text-slate-900 dark:text-white">Competency Self-Assessment</h1>
                <p className="mt-2 text-lg text-slate-600 dark:text-slate-400">Rate your proficiency in each area to receive a personalized development plan from our AI coach.</p>
                <button
                    ...
                    <div className="flex-grow overflow-y-auto prose dark:prose-invert max-w-none prose-headings:text-primary-600 dark:prose-headings:text-primary-400 prose-strong:text-slate-800 dark:prose-strong:text-slate-100 prose-li:marker:text-primary-500">
                        {isLoading && !developmentPlan && (
                             <div className="flex flex-col items-center justify-center h-full text-center text-slate-500">
                                ...
                                <p>Our AI coach is analyzing your assessment and crafting your personalized plan...</p>
                            </div>
                        )}
            ...
            <GuidanceModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
        </div>
    );
};
